module.exports = function(app, passport, connection) {
    ...
    app.post('/staff/:command', (req,res) => {
        ...
        if (req.params.command == 'insert') {
            ...
            var sql = "UPDATE staff SET full_name = ?, username = ?, vnu_email = ?, staff_type = ?, address = ?;";
            connection.query(sql, [req.body.full_name, req.body.username, req.body.vnu_email, req.body.staff_type, req.body.address], (err) => {
                if (err) {
                    throw err;
                    res.send({message:'error'});;
                } else res.send({message:'success'});
